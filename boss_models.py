            ...
            for block_idx in range(num_blocks):
                ...
                if block_encoding[block_idx] == 0:
                    ...
                    blocks.append(ResConv(
                        ...
                        drop_path=DropPath(block_dpr) if block_dpr > 0. else None, avg_down=avg_down, act_layer=act_layer, attn_layer=attn_layer))
...
class BossNet(nn.Module):
    ...
    def __init__(self, encoding=None, num_classes=1000, in_chans=3,
                 ...
                 act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d, aa_layer=None, attn_layer=None, drop_rate=0.0, drop_path_rate=0.,
        ...
        stage_modules, stage_feature_info = make_boss_blocks(
            ...
            drop_block_rate=drop_block_rate, drop_path_rate=drop_path_rate, attn_layer=attn_layer, last_stride=last_stride, **block_args)
...
def bossnet_T0(pretrained=False, **kwargs):
    ...
    """
    model = BossNet(encoding=[[1], [1], [1, 1, 1, 0, 1], [0, 0, 0, 1, 0, 0, 0, 0, 0]], avg_down=True, act_layer=nn.SiLU, attn_layer='se')
...
def bossnet_T0_nose(pretrained=False, **kwargs):
    ...
    stem_width=32, stem_type='deep'
    """
    model = BossNet(encoding=[[1], [1], [1, 1, 1, 0, 1], [0, 0, 0, 1, 0, 0, 0, 0, 0]], avg_down=True, act_layer=nn.ReLU, attn_layer=None)
    model.default_cfg = _cfg()
    return model
...
def bossnet_T1(pretrained=False, **kwargs):
    ...
    stem_width=32, stem_type='deep'
    """
    model = BossNet(encoding=[[1], [1], [1, 1, 1, 0, 1], [0, 0, 0, 1, 0, 0, 0, 0, 0]], avg_down=True, act_layer=nn.SiLU, attn_layer='se', last_stride=1)
    model.default_cfg = _cfg()
    return model
